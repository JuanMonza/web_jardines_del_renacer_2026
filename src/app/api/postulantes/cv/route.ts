...
export async function POST(request: NextRequest) {
  ...
  const ext = path.extname(fileName).toLowerCase().replace(/[^.a-z0-9]/g, '') || '.pdf'; const name = `cv-${session.candidateId}-${Date.now()}${ext}`;
  ...
  await execute('UPDATE candidatos SET cv_url = ? WHERE id = ? AND deleted_at IS NULL', [url, session.candidateId]);
