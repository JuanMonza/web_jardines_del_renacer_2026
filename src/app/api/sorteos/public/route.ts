  ...
  try {
    ...
    return NextResponse.json({
      data: rows.map((row) => ({
        ...
        image: row.imagen ?? "/images/sorteos_img/images.jpg",
        winner: row.nombre
          ? {
              name: repairMojibake(row.nombre),
              ticket: repairMojibake(row.numero_contrato) || undefined,
            }
          : undefined,
      })),
    });
    ...
    return NextResponse.json(
      { data: null, message: "Sin sorteos publicados en base de datos." },
      { status: 503 },
    );
  }
}
