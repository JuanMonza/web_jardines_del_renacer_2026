...
export async function PATCH(
  ...
  try {
    const { id } = await params;
    const body = (await request.json()) as UpdateEstadoPayload;

    const estadosValidos = ['nuevo', 'contactado', 'en_negociacion', 'convertido', 'descartado'];

    if (!body.estado || !estadosValidos.includes(body.estado)) {
      return NextResponse.json(
        { ok: false, message: `Estado inválido. Valores permitidos: ${estadosValidos.join(', ')}.` },
        { status: 400 },
      );
    }
