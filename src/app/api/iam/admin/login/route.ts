...
export async function POST(request: NextRequest) {
  try {
    ...
    response.cookies.set(ADMIN_SESSION_COOKIE, await signAdminSession(session), { httpOnly: true, sameSite: 'lax', secure: process.env.NODE_ENV === 'production', path: trainingCookiePath(), maxAge: 60 * 60 * 8 });
    return response;
