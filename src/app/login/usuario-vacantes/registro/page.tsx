...
function RegistroPostulanteForm() {
  ...
  const update = (key: keyof typeof form, value: string) => {
    const normalized = key === 'documentNumber' || key === 'phone'
      ? value.replace(/\D/g, '')
      : key === 'firstName'
  ...
  const loginHref = `/login/usuario-vacantes${nextPath?.startsWith('/') ? `?next=${encodeURIComponent(nextPath)}` : ''}`;
  return <AuthLoginLayout title="Crea tu cuenta con correo" subtitle="Tu correo será tu acceso para postularte y consultar el estado de tu proceso." sectionLabel="Portal de postulantes"><form onSubmit={submit} className="space-y-4"><LoginTextField label="Documento" inputMode="numeric" value={form.documentNumber} onChange={(e) => update('documentNumber', e.target.value)} required /><LoginTextField label="Nombre completo" value={form.firstName} onChange={(e) => update('firstName', e.target.value)} required /><div><LoginTextField label="Correo electrónico" type="email" value={form.email} onChange={(e) => update('email', e.target.value)} required /><p className="mt-2 text-xs leading-relaxed text-white/75">Usaremos este correo para que ingreses nuevamente y recibas novedades de tu postulación.</p></div><LoginTextField label="Teléfono" type="tel" inputMode="numeric" value={form.phone} onChange={(e) => update('phone', e.target.value)} required /><LoginTextField label="Crea una contraseña" type="password" value={form.password} onChange={(e) => update('password', e.target.value)} required />{error && <p className="rounded-xl border border-red-400/40 bg-red-50 px-3 py-2 text-sm text-red-700">{error}</p>}<button type="submit" disabled={loading} className="w-full rounded-xl bg-black py-3.5 text-lg font-semibold text-white transition hover:bg-black/85 disabled:opacity-60">{loading ? 'Creando cuenta...' : 'Crear cuenta con correo'}</button><Link href={loginHref} className="block text-center text-sm text-[#2f5bd6] hover:underline">Ya tengo una cuenta</Link></form></AuthLoginLayout>;
}
