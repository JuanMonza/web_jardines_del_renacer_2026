...
type AdminUser = { id: number; cedula: string; nombres: string; apellidos: string; email: string; activo: number; ultimo_login: string | null; bloqueado_hasta: string | null; roles: string | null; roleIds: string | null };
type Role = { id: number; nombre: string; descripcion: string | null; color: string | null };
type AdminForm = { cedula: string; nombres: string; apellidos: string; email: string; password: string; roleIds: string[]; activo: boolean };
const emptyForm: AdminForm = { cedula: '', nombres: '', apellidos: '', email: '', password: '', roleIds: [], activo: true };
...
export default function DashboardUsuariosPage() {
  ...
  const openCreate = () => { setEditingUser(null); setForm({ ...emptyForm, roleIds: roles[0] ? [String(roles[0].id)] : [] }); setShowPassword(false); setShowForm(true); };
  const openEdit = (user: AdminUser) => {
    setEditingUser(user);
    setForm({ cedula: user.cedula, nombres: user.nombres, apellidos: user.apellidos, email: user.email, password: '', roleIds: user.roleIds?.split(',').filter(Boolean) || [], activo: Boolean(user.activo) });
  ...
  const saveUser = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      if (!form.roleIds.length) throw new Error('Selecciona al menos un panel para el administrador.');
      const response = await fetch(appUrl(editingUser ? `/api/iam/admin/users/${editingUser.id}` : '/api/iam/admin/users'), { method: editingUser ? 'PATCH' : 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ ...form, roleIds: form.roleIds.map(Number) }) });
  ...
  const exportUsers = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (!response.ok) {
        const payload = await response.json() as { message?: string };
        throw new Error(payload.message || 'No fue posible descargar el directorio.');
      }
      const url = URL.createObjectURL(await response.blob());
      const anchor = document.createElement('a');
      anchor.href = url;
      anchor.download = 'administradores-jardines-del-renacer.xlsx';
      ...
      setShowExport(false);
      setExportPassword('');
      notify(true, 'Directorio descargado. Las contraseñas no se incluyen por seguridad.');
    } catch (error) {
      notify(false, error instanceof Error ? error.message : 'No fue posible descargar el directorio.');
    } finally {
      setExporting(false);
    }
  };
  ...
  return <div className="p-5 md:p-8">
    <TrainingHint text="Descargar administradores genera un Excel sin contraseñas. Nuevo administrador crea una cuenta y permite asignarle uno o varios paneles." className="block">
    <header className="relative overflow-hidden rounded-[28px] border border-white/80 bg-white/55 px-6 py-7 shadow-[0_22px_56px_-38px_rgba(13,54,109,0.8)] backdrop-blur-xl md:px-8">
      ...
      <div className="relative flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between"><div><p className="text-xs font-bold uppercase tracking-[.18em] text-primary">Identidad y acceso</p><h1 className="mt-2 text-3xl font-bold tracking-tight text-text md:text-4xl">Usuarios administrativos</h1><p className="mt-2 max-w-2xl text-sm leading-6 text-textLight md:text-base">Gestiona las cuentas reales de los administradores, sus roles y el acceso seguro a cada operación.</p></div><div className="flex flex-wrap gap-2"><button onClick={() => setShowExport(true)} disabled={loading} className="inline-flex items-center justify-center gap-2 rounded-xl border border-primary/25 bg-white px-5 py-3 text-sm font-bold text-primary transition hover:bg-primary/5 disabled:opacity-50"><Download className="h-4 w-4" /> Descargar administradores</button><button onClick={openCreate} disabled={loading || !roles.length} className="inline-flex items-center justify-center gap-2 rounded-xl bg-primary px-5 py-3 text-sm font-bold text-white shadow-[0_12px_24px_-14px_rgba(29,78,157,0.9)] transition hover:-translate-y-0.5 hover:bg-[#254e92] disabled:cursor-not-allowed disabled:opacity-50"><Plus className="h-4 w-4" /> Nuevo administrador</button></div></div>
    </header>
    </TrainingHint>
    ...
    <AllyReportAccessManager />
    <TrainingHint text="Busca una cuenta por nombre, rol o cédula. Ver panel abre su módulo; editar cambia sus datos y desactivar impide nuevos accesos sin borrar la trazabilidad." className="block">
    ...
    {showExport && <div className="fixed inset-0 z-[100] flex items-center justify-center bg-[#102e55]/55 p-4 backdrop-blur-md" role="dialog" aria-modal="true" aria-labelledby="export-admin-title"><form onSubmit={exportUsers} className="w-full max-w-md rounded-[28px] border border-white/80 bg-[#f8fbff] p-6 shadow-2xl"><div className="flex items-start justify-between gap-4"><div><p className="text-xs font-bold uppercase tracking-[.16em] text-primary">Exportación protegida</p><h2 id="export-admin-title" className="mt-2 text-xl font-bold text-text">Descargar administradores</h2></div><button type="button" onClick={() => { setShowExport(false); setExportPassword(''); }} disabled={exporting} className="rounded-lg p-2 text-textLight hover:bg-primary/10" aria-label="Cerrar"><X className="h-5 w-5" /></button></div><p className="mt-3 text-sm leading-6 text-textLight">El Excel incluirá cédulas, correos, nombres, roles, estado y fechas. Las contraseñas no se pueden recuperar y no se exportan.</p><label className="mt-5 block text-sm font-semibold text-text">Confirma tu contraseña<input autoFocus required type="password" autoComplete="current-password" value={exportPassword} onChange={event => setExportPassword(event.target.value)} className="mt-2 w-full rounded-xl border border-[#cbd9e8] bg-white px-4 py-3 outline-none focus:border-primary" /></label><div className="mt-6 flex justify-end gap-2"><button type="button" onClick={() => { setShowExport(false); setExportPassword(''); }} disabled={exporting} className="rounded-xl border border-[#cfddef] bg-white px-4 py-2.5 text-sm font-bold text-[#42658d]">Cancelar</button><button type="submit" disabled={exporting || !exportPassword} className="inline-flex items-center gap-2 rounded-xl bg-primary px-4 py-2.5 text-sm font-bold text-white disabled:opacity-50">{exporting ? <Loader2 className="h-4 w-4 animate-spin" /> : <Download className="h-4 w-4" />} Descargar Excel</button></div></form></div>}
    {showForm && <div className="fixed inset-0 z-[70] flex items-center justify-center bg-[#102e55]/45 p-4 backdrop-blur-md"><form onSubmit={saveUser} className="max-h-[92vh] w-full max-w-2xl overflow-y-auto rounded-[28px] border border-white/80 bg-[#f8fbff]/95 p-6 shadow-[0_28px_80px_-28px_rgba(8,37,88,0.85)] md:p-7"><div className="flex items-start justify-between gap-4"><div><p className="text-xs font-bold uppercase tracking-[.16em] text-primary">Administración segura</p><h2 className="mt-1 text-2xl font-bold text-text">{editingUser ? 'Editar administrador' : 'Crear administrador'}</h2><p className="mt-1 text-sm text-textLight">Una sola cuenta puede ingresar a todos los paneles seleccionados.</p></div><button type="button" onClick={() => setShowForm(false)} className="rounded-xl border border-[#d6e2f2] bg-white/80 p-2 text-textLight hover:text-primary"><X className="h-5 w-5" /></button></div><div className="mt-6 grid gap-4 md:grid-cols-2"><Field label="Nombres"><input required value={form.nombres} onChange={(event) => setForm((current) => ({ ...current, nombres: event.target.value }))} className="field" /></Field><Field label="Apellidos"><input required value={form.apellidos} onChange={(event) => setForm((current) => ({ ...current, apellidos: event.target.value }))} className="field" /></Field><Field label="Cédula"><input required disabled={Boolean(editingUser)} inputMode="numeric" value={form.cedula} onChange={(event) => setForm((current) => ({ ...current, cedula: event.target.value.replace(/\D/g, '') }))} className="field disabled:cursor-not-allowed disabled:bg-slate-100" /></Field><Field label="Correo corporativo"><div className="relative"><Mail className="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-[#7891af]" /><input required type="email" value={form.email} onChange={(event) => setForm((current) => ({ ...current, email: event.target.value }))} className="field pl-10" /></div></Field><Field label={editingUser ? 'Nueva contraseña (opcional)' : 'Contraseña temporal'}><div className="relative"><input required={!editingUser} type={showPassword ? 'text' : 'password'} value={form.password} onChange={(event) => setForm((current) => ({ ...current, password: event.target.value }))} className="field pr-11" /><button type="button" onClick={() => setShowPassword((current) => !current)} className="absolute right-2 top-1/2 -translate-y-1/2 rounded-lg p-2 text-[#6683a6] hover:bg-primary/10 hover:text-primary">{showPassword ? <EyeOff className="h-4 w-4" /> : <Eye className="h-4 w-4" />}</button></div><p className="mt-1 text-xs text-textLight">Mínimo 12 caracteres.</p></Field><fieldset className="md:col-span-2"><legend className="text-sm font-semibold text-[#294a70]">Paneles autorizados</legend><p className="mt-1 text-xs text-textLight">Selecciona uno o varios. Los accesos actuales se conservan mientras permanezcan marcados.</p><div className="mt-3 grid gap-2 sm:grid-cols-2">{roles.map((role) => { const checked = form.roleIds.includes(String(role.id)); return <label key={role.id} className={`flex cursor-pointer items-start gap-3 rounded-xl border p-3 transition ${checked ? 'border-primary/45 bg-[#eaf2ff]' : 'border-[#d6e2f2] bg-white/75 hover:border-primary/25'}`}><input type="checkbox" checked={checked} onChange={() => toggleRole(String(role.id))} className="mt-1 h-4 w-4 accent-[#315fa8]" /><span><span className="block text-sm font-bold text-[#294a70]">{role.nombre}</span>{role.descripcion && <span className="mt-0.5 block text-xs leading-5 text-textLight">{role.descripcion}</span>}</span></label>; })}</div>{!form.roleIds.length && <p className="mt-2 text-xs font-semibold text-red-600">Selecciona al menos un panel.</p>}</fieldset></div>{editingUser && <label className="mt-5 flex items-center gap-3 rounded-xl border border-[#dce8f6] bg-white/75 px-4 py-3 text-sm font-semibold text-text"><input type="checkbox" checked={form.activo} onChange={(event) => setForm((current) => ({ ...current, activo: event.target.checked }))} className="h-4 w-4 accent-[#315fa8]" /> Mantener cuenta activa</label>}<div className="mt-6 flex flex-col-reverse gap-2 sm:flex-row sm:justify-end"><button type="button" onClick={() => setShowForm(false)} className="rounded-xl border border-[#cfddef] bg-white px-4 py-3 text-sm font-bold text-[#42658d]">Cancelar</button><button disabled={saving || !form.roleIds.length} type="submit" className="inline-flex items-center justify-center gap-2 rounded-xl bg-primary px-5 py-3 text-sm font-bold text-white disabled:opacity-60">{saving && <Loader2 className="h-4 w-4 animate-spin" />}{editingUser ? 'Guardar cambios' : 'Crear administrador'}</button></div></form></div>}
...
function getPanelLinks(roles: string | null) {
  const assigned = roles?.split('|').map((role) => role.trim()) || [];
  const links = [
    assigned.includes('Administrador General') && { href: '/dashboard', label: 'Administración' },
    assigned.includes('Administrador de Vacantes') && { href: '/dashboard-vacantes', label: 'Talento humano' },
    assigned.includes('Administrador de Aliados') && { href: '/dashboard-aliados', label: 'Aliados' },
    assigned.includes('Administrador de Sedes') && { href: '/dashboard-sedes', label: 'Sedes' },
    assigned.includes('Administrador de Talleres') && { href: '/dashboard-talleres', label: 'Talleres' },
    assigned.includes('Administrador de Sorteos') && { href: '/dashboard-sorteos', label: 'Mercadeo' },
    (assigned.includes('Coordinador de Cotizaciones') || assigned.includes('Gestor de Cotizaciones')) && { href: '/dashboard/cotizaciones', label: 'Cotizaciones' },
  ].filter((link): link is { href: string; label: string } => Boolean(link));
  return links.length ? links : [{ href: '/dashboard', label: 'Ver panel' }];
}
function UserCard({ user, onEdit, onDelete }: { user: AdminUser; onEdit: () => void; onDelete: () => void }) { const active = Boolean(user.activo); const panelLinks = getPanelLinks(user.roles); return <article className="group rounded-2xl border border-[#dce7f4] bg-white/70 p-4 transition duration-200 hover:-translate-y-0.5 hover:border-[#a9c6ea] hover:shadow-[0_18px_38px_-28px_rgba(13,54,109,.9)]"><div className="flex gap-3"><div className="flex h-11 w-11 shrink-0 items-center justify-center rounded-2xl bg-[#eaf2ff] font-bold text-[#315fa8]">{`${user.nombres.charAt(0)}${user.apellidos.charAt(0)}`.toUpperCase()}</div><div className="min-w-0 flex-1"><div className="flex flex-wrap items-start justify-between gap-2"><div><h3 className="font-bold text-text">{user.nombres} {user.apellidos}</h3><p className="mt-0.5 truncate text-sm text-textLight">{user.email}</p></div><span className={`rounded-full px-2.5 py-1 text-xs font-bold ${active ? 'bg-emerald-100 text-emerald-700' : 'bg-slate-100 text-slate-600'}`}>{active ? 'Activo' : 'Inactivo'}</span></div><div className="mt-3 flex flex-wrap gap-2"><span className="rounded-lg bg-[#edf3fc] px-2.5 py-1 text-xs font-semibold text-[#42658d]">{user.roles || 'Sin rol'}</span><span className="rounded-lg bg-slate-100 px-2.5 py-1 text-xs text-slate-600">CC {user.cedula}</span></div><div className="mt-3 flex flex-wrap items-center justify-between gap-3 border-t border-[#e4ebf5] pt-3"><p className="text-xs text-textLight">Último ingreso: {formatDate(user.ultimo_login)}</p><div className="flex flex-wrap items-center justify-end gap-1">{panelLinks.map((panel) => <Link key={panel.href} href={panel.href} className="rounded-lg bg-[#edf3fc] px-2.5 py-2 text-xs font-bold text-primary hover:bg-primary hover:text-white">{panel.label}</Link>)}<button onClick={onEdit} className="rounded-lg p-2 text-[#42658d] hover:bg-[#eaf2ff] hover:text-primary" title="Editar administrador"><Edit3 className="h-4 w-4" /></button><button onClick={onDelete} className="rounded-lg p-2 text-[#a3475b] hover:bg-red-50 hover:text-red-600" title="Desactivar administrador"><Trash2 className="h-4 w-4" /></button></div></div></div></div></article>; }
