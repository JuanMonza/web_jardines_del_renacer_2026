...
function PostulanteDashboardContent() {
  ...
  useEffect(() => {
    ...
    async function fetchPortalData() {
      try {
        ...
        const profileResult =
          (await profileResponse.json()) as ApiResponse<CandidateProfile>;
        const applicationsResult =
          (await applicationsResponse.json()) as ApiResponse<JobApplication[]>;
        ...
        if (
          ...
          throw new Error(
            profileResult.message || "No se pudo cargar tu perfil.",
          );
        }
        ...
        if (vacanciesResponse.ok) {
          const vacanciesResult =
            (await vacanciesResponse.json()) as JobVacancy[];
  ...
  const handleSaveProfile = async (event: React.FormEvent) => {
    ...
    try {
      ...
      setNotice({
        ...
        description:
          "Tus datos profesionales quedaron guardados correctamente.",
        variant: "success",
      });
    } catch (error) {
      const message =
  ...
  const handlePhotoUpload = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file.size > 4 * 1024 * 1024) {
      setNotice({
        title: "Fotografía muy pesada",
        description: "La fotografía debe pesar máximo 4 MB.",
        variant: "error",
      });
      return;
    }
    ...
    reader.onload = async () => {
      try {
        ...
        if (!response.ok || !result.success || !result.data)
          throw new Error(
            result.message || "No fue posible cargar la fotografía.",
          );
        setProfile((current) => ({ ...current, photoUrl: result.data!.url }));
        setNotice({
          title: "Fotografía actualizada",
          description: "Tu fotografía quedó guardada en tu perfil.",
          variant: "success",
        });
      } catch (error) {
        setNotice({
          title: "No fue posible cargar la fotografía",
          description:
  ...
  const handleApplyToSelectedVacancy = async () => {
    ...
    try {
      ...
      if (!response.ok || !result.success || !result.data) {
        throw new Error(
          result.message || "No fue posible registrar tu postulación.",
        );
      }
      ...
      setNotice({
        title: "No fue posible postularte",
        description: message,
        variant: "error",
      });
    } finally {
    ...
    <>
      ...
      <section className="bg-gradient-to-b from-[#eef5ff] to-white py-12">
        <Container maxWidth="2xl">
          <section className="mb-8 flex flex-wrap items-center justify-between gap-5 rounded-[28px] border border-white/80 bg-white/70 p-6 shadow-[0_14px_36px_rgba(35,79,132,0.12)] backdrop-blur-xl">
            <div className="flex items-center gap-4">
              <div className="flex h-16 w-16 items-center justify-center overflow-hidden rounded-2xl bg-gradient-to-br from-primary to-[#7ca5d4] text-2xl font-bold text-white shadow-lg">
                {profile.photoUrl ? (
                  <img
                  ...
                  (profile.fullName || "P").trim().charAt(0).toUpperCase()
                )}
              </div>
              <div>
                ...
                <h1 className="mt-1 text-2xl font-bold text-text">
                  Hola, {profile.fullName?.split(" ")[0] || "postulante"}
                </h1>
                <p className="mt-1 text-sm text-textLight">
                  Mantén tu perfil listo para nuevas oportunidades.
                </p>
              </div>
            </div>
            <div className="rounded-2xl border border-primary/10 bg-primary/5 px-5 py-3">
          ...
          <div className="grid grid-cols-1 gap-8 xl:grid-cols-[0.9fr_1.1fr]">
            ...
            >
              ...
              <div className="space-y-3">
                <div className="flex flex-wrap items-center gap-4 rounded-2xl border border-dashed border-primary/25 bg-primary/[0.03] p-4">
                  <div className="flex h-16 w-16 items-center justify-center overflow-hidden rounded-2xl bg-primary/10 text-xl font-bold text-primary">
                    {profile.photoUrl ? (
                      <img
                        src={profile.photoUrl}
                  ...
                  <label className="inline-flex cursor-pointer items-center gap-2 rounded-xl bg-primary px-4 py-2.5 text-sm font-bold text-white transition hover:bg-primaryDark">
                ...
                <details
                  open
                  className="group rounded-2xl border border-primary/10 bg-white/55 p-4"
                >
                  ...
                  <div className="mt-4">
                    <div className="rounded-2xl border border-dashed border-primary/25 bg-primary/[0.03] p-4">
                      ...
                      <input
                        type="file"
                        accept=".pdf,.doc,.docx"
                        className="mt-3 block w-full text-sm text-textLight file:mr-3 file:rounded-lg file:border-0 file:bg-primary file:px-3 file:py-2 file:text-xs file:font-bold file:text-white"
                        onChange={(event) => {
                          const file = event.target.files?.[0];
                          ...
                          reader.onload = async () => {
                            ...
                            if (response.ok && result.success && result.data) {
                              ...
                              setFeedback(
                                "Hoja de vida cargada y guardada correctamente.",
                              );
                              setNotice({
                                title: "Hoja de vida cargada",
                                description:
                                  "Tu CV quedó asociado a tu perfil y estará disponible para el proceso de selección.",
                                variant: "success",
                              });
                            } else {
                              setFeedback(
                              ...
                              setNotice({
                                title: "No fue posible cargar la hoja de vida",
                                description:
                      ...
                      {profile.resumeFileName && (
                        <p className="mt-2 text-xs font-semibold text-emerald-700">
                          CV disponible: {profile.resumeFileName}
                        </p>
                      )}
                    </div>
                  </div>
            ...
            <div className="space-y-6">
              {selectedVacancy && (
                <section className="rounded-[24px] border border-primary/20 bg-gradient-to-br from-white to-[#eef5ff] p-5 shadow-[0_14px_36px_rgba(35,79,132,0.12)] md:p-6">
                  <div className="flex items-start gap-3">
                    <span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-2xl bg-primary text-white shadow-sm">
                    ...
                    <div className="min-w-0">
                      ...
                      <h2 className="mt-1 text-xl font-display text-text">
                        {selectedVacancy.title}
                      </h2>
                      <p className="mt-2 flex items-center gap-2 text-sm text-textLight">
                        <MapPin size={15} />
              ...
              <section className="h-fit rounded-[24px] border border-white/80 bg-white/70 p-5 shadow-[0_14px_36px_rgba(35,79,132,0.12)] backdrop-blur-xl md:p-6">
                ...
                {applications.length === 0 ? (
                  <div className="rounded-2xl border border-dashed border-primary/20 bg-primary/[0.035] p-6 text-sm text-textLight">
                    Aún no tienes postulaciones activas. Cuando encuentres una
                    vacante de tu interés, tu seguimiento aparecerá aquí.
                  </div>
                ) : (
                  <div className="space-y-3">
