...
export async function sendCandidateApplicationReceivedEmail({
  ...
  await transporter.sendMail({
    ...
    html: `<div style="font-family:Arial,sans-serif;color:#24344d;max-width:540px;margin:auto;padding:28px"><p style="color:#3c60a2;font-weight:700;letter-spacing:1.5px;font-size:12px">PORTAL DE POSTULANTES</p><h1 style="font-size:26px;margin:0 0 16px">Recibimos tu postulación</h1><p>Hola, <strong>${recipientName}</strong>.</p><p>Confirmamos que tu postulación para <strong>${position}</strong> fue recibida correctamente por el equipo de Talento Humano.</p><div style="margin:24px 0;padding:16px 18px;border-radius:12px;background:#edf3fc"><p style="margin:0 0 8px"><strong>Estado actual:</strong> Recibida</p><p style="margin:0"><strong>Código de seguimiento:</strong> ${tracking}</p></div><p>Te notificaremos en este correo cuando tu proceso avance a una nueva etapa.</p><p style="font-size:13px;color:#667085">Este es un mensaje automático; por favor no respondas a este correo.</p></div>`,
  });
  return true;
}
