...
export default function PostulantesAdminPanel() {
    ...
    return (
        ...
        <div className="space-y-6">
            ...
            {isModalMounted && selectedApplication && createPortal(<div className="fixed inset-0 z-[2147483647] flex items-center justify-center bg-[#07182e]/55 p-4 backdrop-blur-sm" onClick={() => setSelectedApplication(null)}><section className="flex max-h-[90vh] w-full max-w-5xl flex-col overflow-hidden rounded-[30px] border border-[#dbe5f3] bg-[#f8fbff] shadow-2xl" onClick={(event) => event.stopPropagation()}><header className="flex shrink-0 items-start justify-between border-b border-[#dbe5f3] bg-white px-6 py-5 md:px-8"><div><p className="text-xs font-bold uppercase tracking-[.16em] text-primary">Ficha de candidato</p><h2 className="mt-1 text-2xl font-bold text-text">{selectedApplication.candidateName}</h2><p className="mt-1 text-sm text-textLight">{selectedApplication.vacancyTitle}</p></div><button type="button" onClick={() => setSelectedApplication(null)} className="rounded-xl border border-border bg-white px-4 py-2 text-sm font-bold">Cerrar</button></header><div className="grid overflow-y-auto p-6 md:grid-cols-[1.05fr_.95fr] md:gap-6 md:p-8"><div className="space-y-4 rounded-2xl border border-border bg-white p-5 text-sm"><div className="grid gap-4 sm:grid-cols-2"><div><p className="text-xs font-bold uppercase text-textLight">Correo</p><p className="mt-1 break-all font-semibold text-text">{selectedApplication.candidateEmail}</p></div><div><p className="text-xs font-bold uppercase text-textLight">Estado</p><p className="mt-1 font-semibold text-primary">{selectedApplication.status}</p></div><div><p className="text-xs font-bold uppercase text-textLight">Perfil profesional</p><p className="mt-1 font-semibold text-text">{candidateDetail?.profesion||'No registrado'}</p></div><div><p className="text-xs font-bold uppercase text-textLight">Ubicación</p><p className="mt-1 font-semibold text-text">{candidateDetail?.ciudad||'No registrada'}{candidateDetail?.departamento ? `, ${candidateDetail.departamento}` : ''}</p></div></div>{candidateDetail?.cv_url&&<a href={candidateDetail.cv_url} target="_blank" className="mt-2 block rounded-xl bg-primary px-4 py-3 text-center text-sm font-bold text-white">Abrir hoja de vida</a>}<textarea value={note} onChange={e=>setNote(e.target.value)} placeholder="Nota interna para el proceso" className="mt-5 w-full rounded-xl border border-border bg-[#fbfdff] p-3 text-sm" rows={4}/><button type="button" onClick={() => void updateStatus(selectedApplication.id, selectedApplication.status, note)} className="mt-3 w-full rounded-xl bg-primary px-4 py-3 text-sm font-bold text-white">Guardar nota en trazabilidad</button></div><section className="mt-5 rounded-2xl border border-border bg-white p-5 md:mt-0"><p className="text-xs font-bold uppercase tracking-wider text-primary">Historial del proceso</p><div className="mt-4 space-y-3">{candidateHistory.length ? candidateHistory.map((item,index)=><article key={`${item.created_at}-${index}`} className="border-l-2 border-primary/30 pl-3"><p className="text-xs font-bold text-text">{item.accion.replaceAll('_',' ')}</p><p className="mt-1 text-xs leading-5 text-textLight">{item.descripcion||'Sin detalle.'}</p><p className="mt-1 text-[11px] text-primary">{item.created_at ? new Date(item.created_at).toLocaleString('es-CO') : ''}</p></article>) : <p className="text-sm text-textLight">Cargando movimientos o sin historial disponible.</p>}</div></section></div></section></div>, document.body)}
        </div>
        </>
    );
}
